...
9. **跨 PR 累积的 review 待办（Phase 4 启动前清理 / 或 Phase 5 集中 hotfix）**：
   ...
   **✅ 收口状态（2026-05-30，Phase 5 前清理）**：
   ...
   - **R-A1 ✅ 修复（2026-06-01）**：加 `replay_triggered` state channel（默认
     ...
     - **R-A1-followup（tracked，未做）**：本 PR 仅*发出* `replay_triggered`,
       尚无消费方。补 scorecard/autoresearch 真正*读*它(如区分 first-pass vs
       replayed cycle 的评分/统计)之前,它是"待接入"通道,别让它变 dead channel。
