...
8. **Layer-1 macro tools 缺口**（plan §5.1 列出的工具 vs Phase 0 macro_data 实际
   ...
   | `get_etf_price_data(EEM)` (emerging_markets) | ❌ | 用 `get_north_capital_flow` + `get_us_china_spread`（仍未补）|
   ...
   **✅ 已完成（phase-4-macro-tools，2026-05-29）**：补齐 5 个核心缺口工具
   `get_usdcny` / `get_commodity_prices` / `get_ivx` / `get_etf_indicator` /
   `get_fund_flow`（数据源用户指定：tushare fx_daily / fut_daily / yfinance /
   fund_daily / fund_share），并把 `get_news`(opencli) 接入 news_sentiment。
   13 个 macro 工具全部注册 + 路由 + 测试覆盖。**仍未补**：`get_property_data`
   (china) / `get_us_china_relations` (geopolitical) / `get_caixin_sentiment`
   (news_sentiment) / `get_etf_price_data(EEM/2800.HK)` (emerging_markets) ——
   保留现有替代工具，后续按需补。
