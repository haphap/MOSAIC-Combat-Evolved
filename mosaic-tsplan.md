...
- **Step 3（大件,需用户拍板成本)**:`SwarmEngine` 接口下加 `OasisMiroFishEngine` adapter,
  ...
  - **✅ adapter 已交付(2026-05-31)**:`mosaic/mirofish/oasis.py::OasisMiroFishEngine`
    (SwarmEngine 第三实现,纯 stdlib urllib,deps-light)。`engine='oasis'`(config/RPC)
    路由到它;POST `{MOSAIC_MIROFISH_URL}/scenarios` `{seed,num_days,scenarios,start_prices}`
    → 期望返回 `{scenarios:[montecarlo-shaped...]}`,校验 shape + 盖 `engine='oasis'`,缺 URL/
    超时/非2xx/坏 shape → `MiroFishUnavailable`(handler 映射成清晰 RpcError,**不静默降级**
    以免假冒 oasis 输出)。**契约约定**:报告→我们 shape 的服务端薄翻译由部署方负责(报告
    schema 随引擎版本变,是服务侧的事)。
  - **⛔ 真跑前置(仍需用户)**:本环境无 live 服务,故只做了「请求构造/响应映射/失败降级」
    的 fake-HTTP 单元测试,**未做真实集成**(诚实标注)。真跑需:① Docker 部署 `666ghj/MiroFish`;
    ② 在该服务侧加一个把 MiroFish 报告翻译成我们 scenario shape 的 `/scenarios` 端点;
    ③ `MOSAIC_MIROFISH_URL` + LLM/Zep key + 接受按轮成本。
