...
``PREFLIGHT_REQUIRED`` to ``ACTIVE_VERIFIED`` only after source-specific parser
code and an append-only 30-day preflight receipt are both verified.  This
generator never performs network I/O and never writes source prose.
"""
