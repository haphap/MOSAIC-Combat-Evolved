...
def _check_execution_release() -> Mapping[str, str]:
    ...
    if (
        ...
        or not release["execution_contracts"]
    ):
...
def _check_runtime_contract_rebase_receipt(
    *,
    ...
    if (
        ...
        or authority.get("agent_count") != 25
        or authority.get("execution_stage_count") != 26
        or authority.get("tool_count") != 29
...
def _check_public_boundary() -> str:
    ...
    if runtime_manifest.get("runtime_agent_count") != 25:
        raise ValueError("active runtime Agent roster mismatch")
    if runtime_manifest.get("runtime_stage_count") != 26:
