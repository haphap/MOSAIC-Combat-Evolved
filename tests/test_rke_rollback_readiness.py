...
def test_rollback_readiness_covers_soft_hard_and_compliance_paths():
    ...
    assert checks["compliance_rollback_blocks_runtime_retrieval"].rollback_type == "compliance"
...
def test_rollback_readiness_requires_patch_rollback_rule(tmp_path: Path):
    ...
    patch_check = next(
        check for check in report.checks if check.check_id == "patch_has_slow_decay_rollback_rule"
    )
...
def test_rollback_readiness_reads_monitoring_diagnostics_artifact(tmp_path: Path):
    _copy_registry(tmp_path)
    diagnostics_path = tmp_path / "registry/monitoring/central_bank_monitoring_diagnostics.json"
    ...
    report = build_rollback_readiness_report(tmp_path)
    soft_check = next(
        check for check in report.checks if check.check_id == "soft_rollback_alpha_decay"
    )
...
def test_write_rollback_readiness_report_outputs_registry_artifact(tmp_path: Path):
    ...
    assert (tmp_path / "registry/monitoring/central_bank_rollback_readiness_report.json").exists()
