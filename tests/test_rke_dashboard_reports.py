...
def test_dashboard_report_summarizes_completion_and_monitoring():
    ...
    assert report["source_validation"]["accepted_for_production"] is True
    assert report["source_validation"]["production_blocker_count"] == 0
    ...
    assert report["rule_pack_validation"]["accepted"] is True
    assert report["rule_pack_validation"]["failure_count"] == 0
    assert report["rule_pack_validation"]["record_count"] == 4
    ...
    assert report["manual_review_gates"]["gold_review_packet"]["pending_review_rows"] == 0
    assert (
        report["manual_review_gates"]["gold_review_packet"]["candidate_span_ref_count"]
        > 0
    )
    gold_candidate_count = report["manual_review_gates"]["gold_candidate_claims"][
        "candidate_claim_count"
    ]
    assert gold_candidate_count > 0
    assert (
    ...
    assert (
        report["manual_review_gates"]["review_batches"]["gold_set_review_assist_jsonl"]
    ...
    assert report["manual_review_gates"]["review_progress"]["blocker_count"] == 0
    assert (
...
def test_dashboard_markdown_renders_blockers():
    ...
    assert "Source validation production blockers: 0" in markdown
    ...
    assert "Next license review batch rows: 0" in markdown
    assert (
        "Source license review workbook: registry/review_batches/source_license_review_workbook.md"
        in markdown
    )
    assert "Manual review promotion dry-run ready: True" in markdown
    assert "Manual review progress blockers: 0" in markdown
    assert (
    ...
    assert "Rule pack validation failures: 0" in markdown
