...
def _model_config_output_counts(count: int = 459) -> dict:
    return {
...
def _replay_evidence(
    ...
    **overrides: object,
    ...
    row = {
        ...
        "replay_output_count": 27,
        "replay_footprint_count": 27,
...
def test_fixed_episode_manifest_blocks_without_private_prompt_source():
    ...
    assert result["planned_run_count"] == 1836
    assert result["prompt_preflight"]["blocked_count"] == 54
...
def test_all_agent_prompt_provenance_readiness_blocks_missing_source():
    ...
    assert result["prompt_row_count"] == 54
...
def test_all_agent_prompt_provenance_readiness_accepts_private_release_checks(
    ...
    assert result["agent_count"] == 27
    assert result["prompt_row_count"] == 54
    assert result["ready_prompt_row_count"] == 54
    assert result["release_check_count"] == 54
...
def test_all_agent_prompt_provenance_readiness_blocks_extra_release_rows(
    ...
    assert result["readiness_status"] == "blocked_preflight"
    assert "release_checks[55]: unknown agent/lang" in result["blocked_reasons"]
    assert "release_checks[56]: duplicate agent/lang" in result["blocked_reasons"]
    assert result["ready_prompt_row_count"] == 54
    assert result["all_agent_prompt_provenance_ready"] is False
...
def test_fixed_episode_benchmark_evidence_blocks_missing_proof(
    ...
    assert result["required_paired_output_count"] == 1377
    assert result["prompt_source_status"]["blocked_reason"] == (
        "private_prompt_unavailable"
    )
...
def test_capture_agent_claim_footprints_blocks_non_shadow_use_policy(
    tmp_path: Path, monkeypatch
):
    project_root = tmp_path / "project"
    project_root.mkdir()
    ...
    result = dispatch(
        "rke_benchmark.capture_agent_claim_footprints",
        {
            "benchmark_run_id": "bench-bad-use-policy",
            "rows": [
                {
                    "agent": "us_financial_conditions",
                    "as_of_date": "2026-06-18",
                    "claim_type": "macro_series_claim",
                    "target": {"target_type": "macro_series", "target_id": "USDCNY"},
                    "use_policy": "production_signal_allowed",
                }
            ],
        },
    )
    ...
    assert result["capture_status"] == "blocked"
    assert result["captured_count"] == 0
    assert "use_policy must be shadow_agent_claim_footprint_only" in result[
        "failures"
    ][0]
    assert not (project_root / result["private_rows_path"]).exists()
...
def test_capture_agent_claim_footprints_blocks_cross_run_rows(
    ...
    result = dispatch(
        "rke_benchmark.capture_agent_claim_footprints",
        {
            "benchmark_run_id": "bench-capture-run",
            "rows": [
                {
    ...
    assert result["capture_status"] == "blocked"
    assert result["captured_count"] == 0
    assert "benchmark_run_id does not match request" in result["failures"][0]
    assert not (project_root / result["private_rows_path"]).exists()
...
def test_darwinian_autoresearch_manifest_rejects_bool_outcome_metrics(
    tmp_path: Path, monkeypatch
):
    project_root = tmp_path / "project"
    project_root.mkdir()
    ...
    dispatch(
        "rke_benchmark.capture_agent_claim_footprints",
        {
            "benchmark_run_id": "bench-bool-outcome-metrics",
            "rows": [
                {
                    "agent": "us_financial_conditions",
                    "as_of_date": "2026-06-18",
                    ...
                    "rke_context_hash": "b" * 64,
                    **_runtime_context_proof(1),
                    "report_claim_refs": ["forecast_claim:macro-usdcny-bool"],
                    "current_data_confirmed": True,
                }
            ],
        },
    )
    metrics = _downstream_outcome_metrics("bench-bool-outcome-metrics")
    metrics["risk_adjusted_return"] = True

    manifest = dispatch(
        "rke_benchmark.darwinian_autoresearch_input_manifest",
        {
            "benchmark_run_id": "bench-bool-outcome-metrics",
            "downstream_outcome_metrics": metrics,
            "prompt_mutation_provenance": _prompt_mutation_provenance(
                "bench-bool-outcome-metrics"
            ),
        },
    )

    assert manifest["manifest_status"] == "blocked_preflight"
    assert "downstream_outcome_metrics_missing" in manifest["blocked_reasons"]
    outcome = manifest["skill_inputs"]["risk_adjusted_downstream_outcome"]
    assert outcome["status"] == "missing"
    assert "risk_adjusted_return" not in outcome["metrics"]
...
def _mutation_candidate(**overrides):
    row = {
        ...
        "target_scope": "macro",
        "target_component": "macro.us_financial_conditions",
...
def test_delivery_readiness_blocks_list_items_without_identity(
    tmp_path: Path, monkeypatch
):
    project_root = tmp_path / "project"
    project_root.mkdir()
    ...
    manifest = dispatch(
        "rke_benchmark.delivery_readiness",
        {
            "benchmark_run_id": "bench-delivery-direct-list-id",
            "prompt_mutation_release_checks": [
                {"benchmark_run_id": "bench-delivery-direct-list-id"}
            ],
        },
    ...
    assert manifest["readiness_status"] == "blocked_preflight"
    assert (
        "delivery evidence item id missing prompt_mutation_release_checks[0]"
        in manifest["delivery_input_failures"]
    )
    assert (
        "delivery_evidence_input:delivery evidence item id missing prompt_mutation_release_checks[0]"
        in manifest["blocked_reasons"]
    )
            ...
            "all_agent_prompt_release_checks": [
                {
                    "agent": "us_financial_conditions",
                    "lang": "zh",
                    "prompt_file_path": "prompts/mosaic/cohort_default/macro/us_financial_conditions.zh.md",
                    "benchmark_run_id": benchmark_run_id,
                }
            ],
...
def test_delivery_evidence_store_blocks_list_items_without_identity(
    tmp_path: Path, monkeypatch
):
    project_root = tmp_path / "project"
    evidence_dir = project_root / ".mosaic" / "rke" / "all_agent_evolution"
    ...
    record = dispatch(
        "rke_benchmark.record_delivery_evidence",
        {
            "benchmark_run_id": "bench-delivery-list-id",
            "prompt_mutation_release_checks": [
                {"benchmark_run_id": "bench-delivery-list-id"}
            ],
        },
    ...
    assert record["record_status"] == "blocked"
    assert "delivery evidence item id missing prompt_mutation_release_checks[0]" in record[
        "failures"
    ]
    assert audit["evidence_status"] == "blocked"
    assert audit["recorded_keys"] == []
    assert audit["failures"] == [
        "line 1: delivery evidence item id missing patch_activation_evidence[0]"
    ]
    ...
    rows = [
        {
            ...
            "benchmark_run_id": "other-run",
            "evidence": {"paired_output_count": 1377},
        },
        {
            "schema_version": "legacy_delivery_evidence_v0",
