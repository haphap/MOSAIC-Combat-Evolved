...
def test_report_intelligence_derived_refresh_refuses_empty_private_inputs_overwrite(
    ...
    assert result.blocker_count == 1
    assert "private report-intelligence inputs missing" in result.blockers[0]
    assert "report_metadata.jsonl" in result.blockers[0]
    assert extraction_path.read_text(encoding="utf-8") == before_extraction
    assert readiness_path.read_text(encoding="utf-8") == before_readiness
...
def test_basic_derived_refresh_preserves_invalid_inputs(tmp_path: Path, forecast_content):
    ...
    forecast = registry / "report_metadata.jsonl"
...
def test_report_intelligence_basic_refresh_skips_research_chain(tmp_path: Path, monkeypatch):
    ...
    assert set(result.outputs) == {
        "report_metadata", "analytical_footprints", "metric_candidates",
...
def test_report_intelligence_uses_original_markdown_and_writes_cases_only(tmp_path: Path, scope):
    from mosaic.rke.agent_research_context import build_rke_agent_research_context

    source_id = _write_source(tmp_path / "registry/sources/tushare_research_reports.jsonl")
    result = run_report_intelligence_refresh(
        ReportIntelligenceConfig(derived_scope=scope, root=tmp_path, source_ids=(source_id,)),
        downloader=_fake_downloader, converter=_fake_converter, llm_extractor=_fake_llm,
    )
    registry = tmp_path / "registry/report_intelligence"
    assert result.blocker_count == 0
    assert result.selected_reports == result.pdf_ready_count == result.markdown_ready_count == 1
    assert result.llm_processed_reports == result.analytical_footprint_rows == 1
    assert result.forecast_claim_rows == 0
    assert "forecast_claims" not in result.outputs
    assert not (registry / "forecast_claims.jsonl").exists()
    footprints = _read_jsonl(registry / "analytical_footprints.jsonl")
    original = footprints[0]["research_case"]
    assert original["conclusion"] == "High beta may outperform CSI300"
    assert original["invalidation_conditions"] == ["资金面重新收紧"]
    assert len(original["reasoning_chain"]) == 2
    ...
    context = build_rke_agent_research_context(root=tmp_path, agent_id="cio", as_of_date="2099-01-01")
    assert context["context_items"][0]["research_case"] == original
    if scope == "full":
        assert _read_jsonl(registry / "report_forecast_ledger.jsonl") == []
        assert _read_jsonl(registry / "report_outcome_labels.jsonl") == []
        reviews = _read_jsonl(registry / "analytical_footprint_review_template.jsonl")
        assert len(reviews) == 1 and reviews[0]["manual_review_required"] is True
        assert reviews[0]["footprint_correct"] is None
    # Replaying a stale file cannot restore predictions or alter the source case.
    (registry / "forecast_claims.jsonl").write_text("not JSON: retired prediction output")
    refreshed = run_report_intelligence_derived_refresh(ReportIntelligenceConfig(root=tmp_path, derived_scope=scope))
    assert refreshed.blocker_count == 0
    assert refreshed.forecast_claim_rows == 0
    assert "forecast_claims" not in refreshed.outputs
    assert _read_jsonl(registry / "analytical_footprints.jsonl")[0]["research_case"] == original
    assert build_rke_agent_research_context(root=tmp_path, agent_id="cio", as_of_date="2099-01-01")["context_items"][0]["research_case"] == original
...
def test_report_intelligence_evolution_gate_blocks_until_objective_thresholds_pass():
    ...
    next_actions = {
        action["action_id"]: action
        for action in _evolution_gate_cli_summary(gate)["next_actions"]
    }
...
def test_report_intelligence_evolution_gate_rejects_standalone_prediction_context():
    gate = build_report_intelligence_evolution_readiness_gate(
...
def test_merge_report_intelligence_batch_outputs_dedupes_batch_jsonl(
    ...
    for retired in ("forecast_claims.jsonl", "report_outcome_labels.jsonl", "weighted_research_contexts.jsonl"):
        assert retired not in result["outputs"]
        assert not (tmp_path / "registry/report_intelligence" / retired).exists()
...
def test_merge_report_intelligence_batch_outputs_can_replace_source_ids(
    ...
    (registry / "analytical_footprints.jsonl").write_text(
        "\n".join(
            [
                json.dumps({"footprint_id": "FC-KEEP", "source_id": "SRC-0"}),
                json.dumps(
                    {
                        "footprint_id": "FC-OLD",
    ...
    (registry / "report_outcome_labels.jsonl").write_text(
        "\n".join(
            [
                json.dumps({"outcome_label_id": "OL-KEEP", "footprint_id": "FC-KEEP"}),
                json.dumps({"outcome_label_id": "OL-OLD", "footprint_id": "FC-OLD"}),
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    (batch / "analytical_footprints.jsonl").write_text(
        json.dumps({"footprint_id": "FC-NEW", "source_id": "SRC-REPLACE"}) + "\n",
        encoding="utf-8",
    )
    (batch / "report_outcome_labels.jsonl").write_text(
        json.dumps({"outcome_label_id": "OL-NEW", "footprint_id": "FC-NEW"}) + "\n",
        encoding="utf-8",
    )
    ...
    assert _read_jsonl(registry / "analytical_footprints.jsonl") == [
        {"footprint_id": "FC-KEEP", "source_id": "SRC-0"},
        {"footprint_id": "FC-NEW", "source_id": "SRC-REPLACE"},
    ]
    assert _read_jsonl(registry / "report_outcome_labels.jsonl") == [
        {"outcome_label_id": "OL-KEEP", "footprint_id": "FC-KEEP"},
        {"outcome_label_id": "OL-OLD", "footprint_id": "FC-OLD"},
    ]
