...
def _fake_nvidia_smi(path: Path, samples: list[int]) -> Path:
...
def _run_guard(tmp_path: Path, samples: list[int], child_code: str) -> subprocess.CompletedProcess[str]:
    output = tmp_path / "samples.csv"
    nvidia_smi = _fake_nvidia_smi(tmp_path / "nvidia-smi", samples)
...
def test_guard_stops_command_when_headroom_crosses_floor(tmp_path: Path) -> None:
    result = _run_guard(tmp_path, [2048, 900], "import time; time.sleep(10)")

    assert result.returncode == guard.GUARD_EXIT_CODE
    summary = json.loads((tmp_path / "samples.csv.summary.json").read_text())
    assert summary["minimum_free_mib_observed"] == 900
