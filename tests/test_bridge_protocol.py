...
class _BridgeTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._write_china_snapshot()
        self._write_economic_calendar()
    ...
    def _write_china_snapshot(self) -> None:
        snapshot_dir = self._cache_dir / "macro_snapshots" / "2024-06-30"
        snapshot_dir.mkdir(parents=True)
        observations = []
        for series_id in ("cn_gdp", "cn_cpi", "cn_credit", "cn_export", "cn_fiscal"):
            observations.append(
                {
                    "series_id": series_id,
                    ...
                    "evidence_id": f"macro:{series_id}:2024-05:20240612",
                }
            )
        payload = {
        ...
        (snapshot_dir / "china.json").write_text(json.dumps(payload), encoding="utf-8")

    def _write_economic_calendar(self) -> None:
        def fetch(**request):
            values = {
                "date": "20240630",
                "time": "09:30",
        ...
        collect_eco_calendar(
            ...
            end_date="2024-06-30",
            retrieved_at="2024-06-30T10:00:00+08:00",
            store=EconomicCalendarStore(
                self._cache_dir / "economic_calendar" / "eco_cal.sqlite3"
            ),
            currencies=("CNY",),
        )
