...
def test_unsealed_agent_display_narratives_are_not_latest(tmp_path):
    ...
    assert store.append_agent_display_narratives_from_state(_state()) == 27
    assert store.append_agent_display_narratives_from_state(_state()) == 27
    ...
    with store._connect() as conn:
        assert conn.execute("SELECT COUNT(*) FROM agent_display_narratives").fetchone()[0] == 27
...
def test_append_verifies_nested_accepted_payload_projection(tmp_path):
    ...
    store = ScorecardStore(tmp_path / "scorecard.db")
    assert store.append_agent_display_narratives_from_state(state) == 27
    with store._connect() as conn:
        row = dict(
            conn.execute(
