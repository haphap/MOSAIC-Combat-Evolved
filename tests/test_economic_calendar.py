...
def test_collect_eco_cal_is_append_only_pit_and_idempotent(tmp_path: Path) -> None:
    ...
    assert events[0]["event_family"] == "economic_activity"
...
def test_event_identity_does_not_depend_on_other_rows_in_a_later_capture(
    tmp_path: Path,
) -> None:
    store = EconomicCalendarStore(tmp_path / "eco-cal.sqlite3")
    original = _row(event="制造业 PMI")
    collect_eco_calendar(
        lambda **_kwargs: [original],
        start_date="2026-07-01",
        end_date="2026-07-01",
        retrieved_at="2026-07-01T09:00:00+08:00",
        store=store,
        currencies=["CNY"],
    )
    original_id = store.events_as_of("2026-07-01T09:00:00+08:00")[0][
        "calendar_event_id"
    ]
    collect_eco_calendar(
        lambda **_kwargs: [_row(event="CPI"), original],
        start_date="2026-07-01",
        end_date="2026-07-01",
        retrieved_at="2026-07-01T10:00:00+08:00",
        store=store,
    ...
    later = store.events_as_of("2026-07-01T10:00:00+08:00")
    pmi = next(row for row in later if row["normalized_event"] == "制造业 pmi")
    assert pmi["calendar_event_id"] == original_id
    assert pmi["occurrence_key"] == "OCCURRENCE_DATE:2026-07-01"
...
def test_in_batch_conflict_and_truncation_fail_closed(tmp_path: Path) -> None:
    ...
    assert rejected["failure_reason"] == "TRUNCATED_LEAF:20260701:CNY:中国"
    assert truncated_store.events_as_of("2026-07-01T10:00:00+08:00") == []


def test_truncated_leaf_never_invents_country_routes(tmp_path: Path) -> None:
    requests: list[dict[str, str]] = []

    def fetch(**request):
        requests.append(request)
        return [_row(event=f"event-{index}") for index in range(100)]
    ...
    rejected = collect_eco_calendar(
        ...
        end_date="2026-07-01",
        retrieved_at="2026-07-01T10:00:00+08:00",
        store=EconomicCalendarStore(tmp_path / "truncated.sqlite3"),
        currencies=["CNY"],
    )
    assert rejected["status"] == "REJECTED"
    assert requests == [{"date": "20260701", "country": "中国"}]


def test_country_and_currency_route_binding_mismatch_is_rejected(tmp_path: Path) -> None:
    rejected = collect_eco_calendar(
        lambda **_request: [_row(country="美国")],
        start_date="2026-07-01",
        end_date="2026-07-01",
        retrieved_at="2026-07-01T10:00:00+08:00",
        store=EconomicCalendarStore(tmp_path / "route-mismatch.sqlite3"),
        currencies=["CNY"],
    )
    assert rejected["status"] == "REJECTED"
    assert rejected["failure_reason"] == "ROUTE_BINDING_MISMATCH:20260701:CNY:中国"
