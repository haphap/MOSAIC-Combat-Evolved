    ...
    return RouteCoverageReceipt.seal(
        {
            ...
            "window": {
                "start": (cutoff - timedelta(days=30)).isoformat(),
...
def _aggregate_source_receipt(
    *,
    ...
    observed_start = window_start.date().isoformat() if publication_count else None
    observed_end = (
        (window_end - timedelta(microseconds=1)).date().isoformat()
    ...
    return SourceCaptureReceipt.seal(
        {
            ...
            "coverage": {
                "requested_start": window_start.date().isoformat(),
...
def materialize_geopolitical_snapshot(
    *,
    ...
    if runtime_manifest["manifest_readiness"] == "READY":
        try:
