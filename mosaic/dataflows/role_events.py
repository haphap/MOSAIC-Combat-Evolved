...
from mosaic.dataflows.cross_runtime_json import canonical_hash as _canonical_hash
...
def _as_of_timestamp(as_of_date: str) -> str:
    ...
    return datetime.combine(
        parsed, _A_SHARE_DECISION_CUTOFF, tzinfo=_SHANGHAI
    ).isoformat()
...
def _macro_owner(event: dict[str, Any]) -> str:
    ...
    if (
        event.get("event_family_classifier_version")
        != ECO_CAL_EVENT_FAMILY_CLASSIFIER_VERSION
    ):
        raise DataVendorUnavailable("eco_cal event-family classifier version mismatch")
    computed_family = classify_eco_cal_event_family(
        str(event.get("normalized_event") or "")
    )
    stored_family = str(event.get("event_family") or "")
    if stored_family and stored_family != computed_family:
        raise DataVendorUnavailable("eco_cal event-family classifier lineage mismatch")
    family = stored_family or computed_family
    if currency == "CNY":
