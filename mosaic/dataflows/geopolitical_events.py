...
def _as_of_end(as_of: str) -> datetime:
    try:
        local_date = date.fromisoformat(as_of)
    except ValueError as exc:
        raise DataVendorUnavailable("geopolitical as_of must be YYYY-MM-DD") from exc
    return datetime.combine(local_date, time.max, tzinfo=_A_SHARE_TIMEZONE).astimezone(
        timezone.utc
    )
...
def validate_geopolitical_manifest(payload: Any) -> dict[str, Any]:
    ...
    all_required_ready = all(
        registration_by_source[source]["registration_status"] == "ACTIVE_VERIFIED"
        for source in REQUIRED_SOURCE_IDS
    )
    expected_readiness = "READY" if all_required_ready else "PREFLIGHT_REQUIRED"
...
def validate_event_revision(
    ...
    for evidence in row["evidence_catalog"]:
        ...
        if evidence.get("published_at") is not None:
            _parse_datetime(evidence["published_at"], "evidence published_at")
    ...
    if row["verification_status"] == "UNCONFIRMED" and provider_kinds != [
        "STRUCTURED_DISCOVERY"
    ]:
...
def build_geopolitical_events_snapshot(
    ...
    *,
    ...
    manifest_ready = manifest["manifest_readiness"] == "READY"
    snapshot = {
        "as_of": as_of,
...
def load_geopolitical_events_snapshot(as_of: str) -> dict[str, Any]:
    ...
    if snapshot["readiness"] != "READY":
        raise DataVendorUnavailable(
            "geopolitical snapshot rejected: required 30-day source preflight or route coverage is incomplete"
        )
    return snapshot
