...
def append_component_calibration_signals(
    ...
    *,
    ...
    if runtime_input.get("agent_id") != agent_id:
        raise ValueError("component calibration runtime owner mismatch")
    ...
    if (
        runtime_input.get("component_weight_contract_version") != component_version
        or accepted.get("component_weight_contract_version") != component_version
    ):
...
def build_component_regime_snapshot(
    *,
    ...
    """Build a PIT regime input; the regime itself is always derived, never supplied."""
    ...
    for item in observations:
        ...
        volatility = _finite(
            item.get("realized_volatility_20d"), "regime.realized_volatility_20d"
        )
        percentile = _finite(
            item.get("trailing_252d_p80"), "regime.trailing_252d_p80"
        )
        if volatility < 0 or percentile < 0:
            raise ValueError("component regime values must be non-negative")
        ...
        normalized.append(
            {
                ...
                "trailing_252d_p80": percentile,
                "regime": "STRESS" if volatility > percentile else "NORMAL",
                "pit_status": "VERIFIED",
                "source_evidence_ids": sorted(set(evidence_ids)),
            }
        )
    normalized.sort(key=lambda row: row["as_of"])
    without_hash = {
...
def _regime_map(snapshot: Mapping[str, Any], cutoff_at: str) -> dict[str, dict[str, Any]]:
    if (
        ...
        or snapshot.get("regime_contract_version")
        != "a_share_realized_volatility_regime_v1"
        or snapshot.get("pit_status") != "VERIFIED"
    ):
    ...
    for item in observations:
        ...
        expected = "STRESS" if volatility > percentile else "NORMAL"
        if item.get("regime") != expected:
            raise ValueError("component regime derivation mismatch")
        available_at = _timestamp(item.get("available_at"), "regime.available_at")
        if available_at > _timestamp(f"{as_of}T15:00:00+08:00", "regime.as_of_close"):
