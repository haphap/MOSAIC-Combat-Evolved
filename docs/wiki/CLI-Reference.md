...
```
Options: `--cohort <name>`, `--date <YYYY-MM-DD>`, `--fake-llm`, `--structured-smoke`, `--llm-provider <name>`, `--model <name>`, `--base-url <url>`, `--max-tokens <count>`, `--prompts-repo <path>`, `--prompts-root <path>`, `--current-positions-json <json>`, `--current-positions-file <path>`, `--paper-positions`, `--paper-execute-deltas`, `--out <path>`. Runs all 28 logical agents through 29 LangGraph.js stages. `--structured-smoke` uses a real structured-output provider with bundled prompts, temperature 0, a default 6144-token completion cap, and no production release, scorecard, outcome, RKE, or paper-order writes.

For a reproducible real-model contract smoke without licensed payloads, build an explicit synthetic PIT bundle first:
...
Use the two environment bindings printed by the builder when invoking `daily-cycle --structured-smoke`. The bundle is marked `SYNTHETIC_NON_PRODUCTION`, contains no vendor prose, and verifies graph/schema/tool wiring only. It does not replace the separate live Tushare permission/schema probe or source-readiness audits.
