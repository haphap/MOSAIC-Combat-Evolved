...
trading day, rejects sndr settings outside the desktop-safe 128K/0.85 envelope, and evaluates
monthly historical Prompt candidates in isolated private branches. Sustained real-model runs
must use the runbook's KDE VRAM guard so the 1 GiB load floor is measured and enforced.
