...
`--checkpoint <path>` stores the accepted Agent-stage prefix, graph state, and accepted structured outputs for one trading day in any daily-cycle mode. `--resume` requires the same `--date`, cohort, and valid stage prefix, then continues without replaying accepted Agents. Use a new checkpoint path on the next trading day. With `--paper-positions --paper-execute-deltas`, that new daily cycle reads the active paper account updated by the previous day.
