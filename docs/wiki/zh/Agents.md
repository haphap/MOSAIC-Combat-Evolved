...
Layer-1 agent 调用 sidecar 工具(Tushare/akshare/FRED/雪球 等)—— 如 `volatility` 用 `get_ivx` + `get_etf_indicator(510050.SH)`;`emerging_markets` 用 `get_etf_price_data`;`china` 用 `get_property_data`(国房景气指数)。macro 层共 17 个工具。
