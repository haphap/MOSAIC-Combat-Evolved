...
```text
darwinian.weight_rewrite_enabled = true by default
```

回滚条件：

- 如需要完全恢复旧 recommendation weighting，可显式设
  `darwinian.weight_rewrite_enabled=false`。
- 回滚后使用旧 `clip(0.5 + rolling_sharpe_30)`，macro rows 不参与统一演化更新。
...
- 保留 feature flag；当前默认开启，必要时可显式关闭回滚。

建议配置：

```text
darwinian.weight_rewrite_enabled = true
...
13. unified darwinian_weights 已实现为默认路径，保留 rollback flag。
...
## Implementation Status: P6-P9

当前分支已按本计划落地 P6-P9，Darwinian rewrite 默认开启并保留回滚开关：

```text
P6 integration/provenance:
  已增加 ingest -> score -> list_macro_skill -> autoresearch.trigger -> record_mutation
  -> evaluate_pending 的跨阶段测试。
  private prompt mutation 会记录 prompt_repo_id / prompt_sha256 / code_commit_hash。
  evaluator 只匹配相同 prompt repo metadata 的 mutation run。
...
P8 influence diagnostics:
  macro_signals ingest 时计算 equal-weight leave-one-out influence_weight_equal。
  scoring 时写 effective_macro_score_5d = influence_weight_equal * raw_macro_score_5d。
  selection 仍使用 mean_raw_macro_score_5d，避免低 influence 坏 agent 死区。
  Darwinian ranking 读取 raw_macro_score_5d，不读取 effective_macro_score_5d。

P9 unified Darwinian weights:
  darwinian_weights schema 已扩展为通用 agent weight table。
  默认 darwinian.weight_rewrite_enabled=true，使用上游式 top quartile *1.05 /
  bottom quartile *0.95 / middle unchanged。
  显式设置 darwinian.weight_rewrite_enabled=false 时回滚到旧 rolling Sharpe -> weight。
...
```bash
uv run pytest -q
cd mosaic-ts && pnpm exec tsc --noEmit
cd mosaic-ts && pnpm exec vitest run
cd mosaic-ts && pnpm exec biome check src test
git diff --check
```
