...
```bash
MOSAIC_RKE_TMPDIR=~/tmp/mosaic-rke TMPDIR=~/tmp/mosaic-rke uv run python -m pytest tests/test_rke_report_intelligence.py -q --basetemp ~/tmp/mosaic-rke/pytest-rke-ri
...
```bash
MOSAIC_RKE_TMPDIR=~/tmp/mosaic-rke TMPDIR=~/tmp/mosaic-rke uv run python -m pytest tests/test_rke_report_intelligence.py -q --basetemp ~/tmp/mosaic-rke/pytest-rke-ri
MOSAIC_RKE_TMPDIR=~/tmp/mosaic-rke TMPDIR=~/tmp/mosaic-rke uv run python -m pytest tests/test_rke_schema_artifacts.py -q --basetemp ~/tmp/mosaic-rke/pytest-rke-schema
uvx ruff@0.15.15 check mosaic tests
git diff --check
```
