...
```bash
MOSAIC_RKE_TMPDIR=.mosaic/tmp TMPDIR=.mosaic/tmp uv run python -m pytest tests/test_rke_report_intelligence.py -q --basetemp .mosaic/tmp/pytest-rke-ri
...
```bash
MOSAIC_RKE_TMPDIR=.mosaic/tmp TMPDIR=.mosaic/tmp uv run python -m pytest tests/test_rke_report_intelligence.py -q --basetemp .mosaic/tmp/pytest-rke-ri
MOSAIC_RKE_TMPDIR=.mosaic/tmp TMPDIR=.mosaic/tmp uv run python -m pytest tests/test_rke_schema_artifacts.py -q --basetemp .mosaic/tmp/pytest-rke-schema
uvx ruff@0.15.15 check mosaic tests
git diff --check
```
