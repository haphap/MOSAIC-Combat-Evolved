...
> `registry/prompt_checks/runtime_agent_manifest_v3.json` 为准；当前角色、数据、Darwinian
> 与 prompt 演化/发布治理以
...
- **Q4（已被实现修订）**：当前 canonical superinvestor roster 为
  Druckenmiller / Munger / Burry / Ackman；以 runtime-agent manifest 为准。
...
**已识别、范围清晰的可补项 —— ✅ 全部交付(2026-06-01)**:
1. **`mirofish` 情景注入通路 ✅** —— `config.mirofish.inject_context`(默认 ON，可显式关闭)把最新情景
   上下文(regime/最高信念方向/tail-risk + as_of_date 防前视)回灌 CRO、autonomous execution 与 CIO prompt(7M Step 2)。
2. **results 导出器 ✅(PR #40)** —— `mosaic/backtest/results_export.py` 把回测产出聚合成
   summary.json / portfolio_trajectory.csv / equity_curve.png(matplotlib 可选);`run_backtest`
   + `backtest.run_historical` RPC 接 `results_dir`。
3. **ETF ingest CLI ✅(PR #35)** —— `pnpm dev data incremental --kind etf` + `data.*` RPC。

**与 ATLAS 的诚实对比一句话**:情景引擎/相关性/打分 ≈ 或优于 ATLAS 公开版,swarm 机制**更真**
(numpy 多 actor 耦合 vs 一次 LLM prompt 扮演);预测回灌 agent prompt 通路已补(候选 #1)。
...
- **7M.1 — 交互引擎（agent-to-agent，自建轻量）✅ 已交付（2026-05-30，分支
  ...
  - **开关（用户要求的 on/off）**：`config.mirofish.engine`（现默认 `oasis`
    连接真实 MOSAIC-Fish；`montecarlo` / `swarm` 为显式本地模式）；`mirofish.generate_scenarios` 加 `engine` 参数覆盖；CLI
...
- **✅ 7M.1b 已完成（2026-05-30）**：扫描 impact 0.08–0.20，选定**默认
  ...
  当时默认-off（montecarlo）的行为不受影响；后续真实 Fish 闭环交付时默认改为 oasis。
...
8. **Layer-1 macro tools 缺口**（plan §5.1 列出的工具 vs Phase 0 macro_data 实际
   ...
   |---|---|---|
   | `get_property_data` (china) | ✅ 已补（akshare 国房景气指数）| wired into china REQUIRED_TOOLS（北向资金已停更,§18 移除）|
   ...
   **✅ 收口（get_property_data，2026-06-01）**：Tushare 宏观确认无房地产专项，
   改用 **akshare `macro_china_real_estate`（国房景气指数，月度 >100 扩张/<100
   收缩）** 作主源。`mosaic/dataflows/macro_data.py:get_property_data(top_n=24)`
   → interface 路由（macro_data 类目，akshare vendor）→ `macro_tools` `@tool`
   `get_property_data` → 接入 `china` REQUIRED_TOOLS（与 get_north_capital_flow
   并列，不再是占位替代）。17 个 macro 工具。
...
9. **跨 PR 累积的 review 待办（Phase 4 启动前清理 / 或 Phase 5 集中 hotfix）**：
   ...
   **✅ 收口状态（2026-05-30，Phase 5 前清理）**：
   ...
   - **R-T1 ✅ 修复（2026-06-01）**：写了 typed `chainEdges` helper
     （`mosaic-ts/src/graph/_edges.ts`，从 graph 自身 `addEdge` 签名推导 edge
     端点类型，无 `any`）。`StateGraph` 原地 mutate 返回 this，故边按副作用累加、
     不再重赋值。去掉全部 6 处 `let graph: any` + biome-ignore（layer1/2/3 用
     chainEdges 循环；layer4 ×2 + daily_cycle 用纯 typed fluent 链）。300 TS 测试
     不变 + `edges.test.ts` 单测 helper。
   - **R-A4 ✅ 文档收口（2026-06-01）**：`ingest_full` docstring 标注 `timeout`
     是 per-Tushare-request 上限（非全 ingest wall-clock）。
   - **R-P1 ✅ 修复（2026-06-01）**：不切 qlib DataApi（与「不 import qlib 读 bin」
     的设计相悖、且会把 qlib.init 拖进轻量 validate 路径）。改为把 bin header 解析
     收敛到 `_qlib_bin_start_index`（finite + 近整数 + 在 [0, calendar_days] 内），
     格式漂移抛 `QlibBinFormatError` 并计入 `validate_after_ingest` 返回的
     `format_errors` + ERROR 日志 —— 由「悄悄读错」变「响亮报错」。
   - **R-A1 ✅ 修复（2026-06-01）**：加 `replay_triggered` state channel（默认
     false，仅 layer4_replay 节点经 invokeSubgraph 的 extra 参数置 true），下游
     可区分首轮 vs 重放周期。daily_cycle 端到端测试两路径断言。
     - **R-A1-followup ✅ 修复（2026-06-01）**：scorecard 现*消费* `replay_triggered`
       —— recommendations 表加 `replay_triggered` 列(+ 旧库 ALTER 迁移),
       `expand_state_to_recommendations` 把该标志盖到每行,`append_from_state`
       upsert 持久化(replay 重入会翻 0→1)。下游可按 first-pass vs replayed 切分
       已落库的建议。不再是 dead channel。
   - **R-A3 ✅ 修复（2026-06-01）**：新增 `backtest_failed_days` 表(run_id FK
     CASCADE, date, error, recorded_at, PK(run_id,date)) + store
     record/get/clear 方法(upsert 幂等);`backtest.record_failed_days` /
     `backtest.get_failed_days`(可带 clear_dates/clear_all)RPC + TS wrapper;
     `backtest-fill` 失败时落库 + 成功日清除,新增 `--retry-failed`(仅重跑已记录
     的失败日),run 仅在无残留失败日时才标记 completed。操作员/未来 autoresearch
     自动重跑可直接查询失败日,不必翻终端日志。
   - **§14 所有 R-* 项已全部收口。** 余下仅 7M.2/7M.3(增益验证 NO-GO)。
...
- **资金流向**(`get_stock_moneyflow` 个股 doc 170 / `get_industry_moneyflow` 行业 doc 343):
  `macro_data.py` 新 impl(`_query_tushare` + `_df_to_markdown_csv`)→ interface
  `VENDOR_METHODS` + `TOOLS_CATEGORIES.macro_data` + 日期路由(个股 `_RANGE (1,2)` / 行业
  `_CURRENT_DATE 0`)→ `default_config`(tushare)→ `macro_tools.py` 2 `@tool`。`net_mf_amount`
  主力净流入(万元)+ 大/特大单 buy/sell(仅留主力两档);个股挂 `institutional_flow`、行业挂
  6 sector;提示词接入(zh+en)(PR #53)。
  - **行业过滤(PR #56,已交付)**:`get_industry_moneyflow` 加可选 `industries`(同花顺行业名子串,
    ASCII/CJK 逗号分隔;子串匹配,故 "医疗" 故意覆盖整族;不匹配降级返回全表 + 提示),各 sector
    提示词写明本行业 THS 名 —— 免去每次拉全 ~90 行业表 + agent 自行 Shenwan↔同花顺行业 映射。
    TODO:THS 行业名现散落硬编码在 12 个 prompt 里,若 THS 改名需逐个改;后续可抽到单一常量映射。
...
- **`get_top_holdings_overlap`**(relationship_mapper 想要的持仓重叠)+
  ...
  `get_stock_research` + `get_etf_holdings` + LHB 近似推断关系。

**移除 —— 北向资金(沪深港通)已停止公布(2026-06):**
- 沪深港通实时额度/资金流向官方已停更,`get_north_capital_flow`(`moneyflow_hsgt`)
  全链路删除:macro_data fn + interface(VENDOR_METHODS/category/_RANGE)+ default_config
  + macro_tools `@tool` + 7 agent REQUIRED_TOOLS(dollar/china/emerging_markets/
  institutional_flow/relationship_mapper/consumer/semiconductor)+ Python/TS 测试。
- **3 个 schema 重设计**(字段含北向语义,不能裸删):
  - `dollar.north_flow_correlation` → **`dxy_cny_correlation`**(USD/CNY vs DXY 相关性,
    用 `get_usdcny` + DXY)。
  - `institutional_flow.north_net_flow_cny` → **`main_net_flow_cny`**(主力净流入,
    来自 #53 `get_stock_moneyflow` 的 net_mf_amount 汇总)。
  - `emerging_markets.hk_a_share_ratio` / `capital_flow`:保留字段名,语义改为
    **跨市场 ETF 价格比**(513050.SH 中概 / 510300.SH 沪深300,用 #50 ETF 工具),
    不再用 north/south 代理。
- 替代信号:个股主力(`get_stock_moneyflow`)、行业资金(`get_industry_moneyflow`)、
  ETF 价格/份额(`get_etf_*` / `get_fund_flow`)、DXY+中美利差。news_sentiment 的
  `contrarian_flag` 改引 institutional_flow 的 `main_net_flow_cny`;china 改用
  `get_property_data` 作国内景气 corroborator。
