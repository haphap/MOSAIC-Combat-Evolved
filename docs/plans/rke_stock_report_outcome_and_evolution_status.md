...
fields, but 16 rows now have stale target hashes against the current footprint
review template. `review-progress --actions-only --review-kind footprint_review`
therefore reports `current_batch_target_aligned=false` and the next action is
to re-run `prepare-footprint-review --limit 50 --offset 0 --overwrite`, which now
selects pending rows rather than absolute target-template rows. The target
