...
The current local expanded gold-set scratch has 30 rows, 0 complete rows, and
30 pending rows; its evidence and target hashes are aligned. The expanded target
review template now has 211 rows, with 158 complete and 53 still pending across
two batches; after the current batch, only the final 3-row batch remains. The
stale full reviewed import must be regenerated after the expanded batches are
manually filled.
...
filtering, producing 184 candidate diagnostics and expanding the local review
template to 211 rows / 68 documents, with 63 documents reviewable before human
decisions.
...
expanded gold-set scratch batch has 30 rows, 0 complete rows, and 30 pending
rows; its private evidence draft is aligned with the same 30 scratch rows. The
promotion gold-set import remains not ready because the expanded current batch
and final 3-row batch still require manual decisions before the full reviewed
import can be regenerated. The previous stale
