...
- 条件 4：部分完成。stock/industry outcome 已按 `label_type`、benchmark/cost model
  ...
  规则；clean validation corpus 仍需重跑以确认原先 `agent_assignment_missing=33` 是否关闭。
  ...
  PIT-safe metadata 字段推导 stock `market_cap_bucket`，从 claim component roles 推导
  industry `industry_cycle_bucket`；clean corpus 仍需补样本或重跑以证明 unknown/missing bucket
  coverage 实际下降。
