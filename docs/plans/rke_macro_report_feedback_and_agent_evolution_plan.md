...
proposed，尚未实现的 stock/industry gate 分支；当前 stock/industry outcome、mapping、
PIT/provenance/privacy evidence 已进入通用 RI-EVOL 检查和现有 readiness/profile/audit，
但还没有独立 RI-STOCK / RI-INDUSTRY check IDs。以下 ID 是后续条件 11/12 的目标：
...
- 条件 7：部分完成。独立 `cross_asset_consistency` 验收已落地：parent claim 根据
  ...
  not_applicable 1702。`macro_agent_research_priors.jsonl` 的 rating buckets 已标准化为
  ...
  pending 1831。这个状态只表示 bucket contract 已落地，不表示宏观 rating 已有充分统计
...
- 条件 9：已满足当前边界。report-intelligence 派生报告已按本地私有处理，不作为默认提交内容；
  ...
  边界内。
- 条件 10：部分完成。`export-macro-agent-priors --agent-id macro.dollar
  --as-of-date 2026-06-18 --no-source-prose` 可输出 539 条 shadow prior，
  `production_signal_allowed=false`，且 `private_text_included=false`。通用 agent context
  builder 已存在，但 stock/industry/macro 的统一 ranked export 仍归入条件 11。
...
- `rke_benchmark.all_agent_prompt_provenance_readiness` 已能把全 25 agents × 2
  ...
  benchmark/replay 前置 gate；release evidence 必须匹配 preflight 的 private
  `prompt_repo_id` 和 `prompt_repo_revision`，并保留 prompt source summary/dirty
  blocker；真实 private prompt release 仍需在 private prompt repo 中完成。
...
- `rke_benchmark.agent_profile_evolution_readiness` 已能检查四层 agent footprint
  ...
  profile update ref 和 evolution input ref；真实 profile/evolution 写入仍需由实际
  benchmark/replay run 产生。
...
宏观研报观点还额外具备可导出的 macro agent shadow research prior 和 RI-MACRO evolution gate。
但三域统一的 agent-facing ranked context、rule/recipe/parameter compiler、所有 agent private
prompt repo 版本的 RKE 驱动演化仍未完成。
...
2. 同步补剩余三域评级缺口：剩余 target-family owning-agent mapping、stock market-cap/fundamental
   ...
   aggregate，后续只需随新 target family 扩展映射。macro `cross_asset_consistency` 已有独立
   条件 7 证据；后续只随 claim-leg coverage 扩样，并继续保留 hard-incompatible 与 50% 冲突阈值审计。
