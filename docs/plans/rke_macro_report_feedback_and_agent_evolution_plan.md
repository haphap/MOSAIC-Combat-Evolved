...
- 条件 4：部分完成。stock/industry outcome 已按 `label_type`、benchmark/cost model
  ...
  PIT-safe metadata 字段推导 stock `market_cap_bucket`，从 claim component roles 和
  standardized metric families 推导 industry `industry_cycle_bucket`；用当前 builder
  重算 clean corpus 时，industry unknown cycle buckets 从旧 artifact 的 58/58 降到
  9/58。stock 仍是 74/74 unknown，因为 clean corpus 未提供市值字段；这保持为数据
  coverage 缺口，不做交易所或代码前缀猜测。
...
2. 同步补剩余三域评级缺口：target-family owning-agent mapping、stock market-cap/fundamental
   buckets 和 industry cycle buckets 的 public builder contract 已补到可测试路径；
