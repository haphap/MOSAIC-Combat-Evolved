...
coupling among **DXY + USD/CNY + north-bound flow** and produce a compact
"dollar – RMB – foreign capital" view.
...
* `get_north_capital_flow(start_date, end_date)` — north-bound A-share net
  flow. When DXY strengthens north flow typically goes net-outflow and
  vice versa; the cleanest "FX vs foreign capital" coupling signal.
...
2. **Quantify**: cite DXY level + WoW BPS change, north-bound weekly net
   in CNY million, CN-US 10Y spread in BPS.
3. **`north_flow_correlation` is the correlation coefficient × 100, integer**
   (e.g. -73 means -0.73). This number drives downstream cro /
...
```json
{
  ...
  "north_flow_correlation": <integer, -100 to 100>,
...
* `cny_pressure = HIGH` only when DXY +1% WoW **and** north flow has been
  net-outflow ≥ 10B CNY for 3 consecutive sessions.
* `cny_pressure = LOW` only when DXY -1% WoW **and** north flow net-inflow
  ≥ 10B CNY consecutively.
