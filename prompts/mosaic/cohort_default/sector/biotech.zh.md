...
工具：只调用 get_sector_research_snapshot、get_broker_research、get_etf_holdings、get_indicators、get_industry_moneyflow、get_industry_policy_digest、get_rke_research_context、get_stock_data、get_supply_chain_evidence；候选域、方向和日期由运行时冻结，不得扩域。
...
必需运行时工具：`get_sector_research_snapshot`, `get_broker_research`, `get_etf_holdings`, `get_indicators`, `get_industry_moneyflow`, `get_industry_policy_digest`, `get_rke_research_context`, `get_stock_data`, `get_supply_chain_evidence`。
