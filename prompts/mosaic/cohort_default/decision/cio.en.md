...
Emit `claims` and top-level `claim_refs`. Every claim must cite catalog `evidence_id` values through `evidence_ids`; every `INTERPRETATION` claim must also cite a permitted opaque identifier through `research_rule_refs`. Every position decision and control resolution must cite supporting claims through `claim_refs`. When evidence is insufficient, use the current stage schema to emit an evidence-backed explicit empty disposition and a `RISK_FLAG` claim. Never invent evidence ids, fingerprints, citation identifiers, or cross-run references.
