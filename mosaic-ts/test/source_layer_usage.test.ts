...
describe("source-layer Darwinian usage", () => {
  ...
  it("represents complete abstention as no directional signal, not fake equal weights", () => {
    const input = state();
    input.layer2_outputs = {
      technology: sectorOutput("technology", {
        selection_status: "NO_QUALIFIED_DIRECTION",
        confidence: 0.8,
      }),
      agriculture: sectorOutput("agriculture", {
        selection_status: "NO_QUALIFIED_DIRECTION",
        confidence: 0.4,
      }),
    };
    ...
    expect(Object.values(receipt.reliability_by_agent).every((row) => row.usage_share === 0)).toBe(
      true,
    );
  });
