...
describe("prompt optimizer public contracts", () => {
  it("covers all 25 Agent-owned champions and rejects the shared CIO proposal stage", () => {
    expect(new Set(RUNTIME_AGENT_STAGE_SPECS.map((row) => row.agent)).size).toBe(25);
    expect(RUNTIME_AGENT_STAGE_SPECS).toHaveLength(26);
