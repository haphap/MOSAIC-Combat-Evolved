 ...
 * Private evolution writes optimized prompts there instead of committing them
 * under the project `prompts/mosaic/**` tree.
 */
...
import { assertPublicBundledCohorts } from "../../agents/prompts/public_prompt_cohort.js";
import {
...
export function registerPrompts(program: Command): void {
  ...
  prompts
    ...
    .action(async (opts: ExportRuntimeManifestOpts) => {
  ...
  prompts
    ...
    .action(async (opts: CheckBundledPromptOpts) => {
      ...
      try {
        ...
        if (opts.json) {
          ...
          console.log(
            color(
              `bundled-prompts ${result.ready ? "ready" : "blocked"} ` +
  ...
  prompts
    ...
    .action(async (opts: SyncMacroPromptsOpts) => {
      ...
      assertPublicBundledCohorts(requested);
...
function parseSelection(value: string | undefined): ReadonlySet<string> | undefined {
