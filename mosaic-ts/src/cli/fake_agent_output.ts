...
export function fakeAgentStructuredOutput(
  ...
  if (sectorAgent) {
    ...
    if (schemaMentionsProperty(jsonSchema, "final_selection")) {
      return { final_selection: fakeSectorSubmission(sectorAgent, messages) };
    }
    return fakeSectorSubmission(sectorAgent, messages);
  }
...
function fakeSectorDirectionResearch(name: StandardSectorAgentId, messages: unknown): unknown {
  ...
  const coverageDirective =
    jsonObjectAfterMarker(text, "Runtime-owned exact role-event coverage directive:") ??
    embeddedJsonObject(text, "coverage_directive");
  const macroCoverage = objectRecord(coverageDirective?.macro_event_fit) ?? {};
  const catalystCoverage = objectRecord(coverageDirective?.catalysts) ?? {};
  const coverageEvidenceIds = [
    ...new Set([
      ...stringValues(macroCoverage.coverage_evidence_ids),
      ...stringValues(catalystCoverage.coverage_evidence_ids),
    ]),
  ];
  const claimCoverageEvidenceIds =
    macroCoverage.coverage_state === "SOURCE_UNAVAILABLE" &&
    catalystCoverage.coverage_state === "SOURCE_UNAVAILABLE"
      ? []
      : coverageEvidenceIds;
  const claimId = `fake-claim-${name}`;
  const directions = STANDARD_SECTOR_ROLE_CONTRACTS[name].directionIds;
  const criterionResults = fakeSectorCriterionResults(
    name,
    claimId,
    evidenceId,
    macroCoverage,
    ...
    comparison_claims: [
      {
        ...
        evidence_ids: [
          ...new Set([...(evidenceId ? [evidenceId] : []), ...claimCoverageEvidenceIds]),
        ],
        research_rule_refs: [],
      },
    ],
...
function fakeSectorCriterionResults(
  ...
  macroCoverage: Record<string, unknown>,
  catalystCoverage: Record<string, unknown>,
): Array<Record<string, unknown>> {
  const macroState =
    typeof macroCoverage.coverage_state === "string"
      ? macroCoverage.coverage_state
      : "COVERAGE_CONFIRMED_NO_MATERIAL_EVENT";
  const catalystState =
    typeof catalystCoverage.coverage_state === "string"
      ? catalystCoverage.coverage_state
      : "COVERAGE_CONFIRMED_NO_MATERIAL_CATALYST";
  const macroEvidenceIds = stringValues(macroCoverage.coverage_evidence_ids);
  const catalystEvidenceIds = stringValues(catalystCoverage.coverage_evidence_ids);
  return [
    ...
    ),
    {
      ...
      coverage_state: macroState,
      comparison_status: macroState === "SOURCE_UNAVAILABLE" ? "UNAVAILABLE" : "COMPARABLE",
      verdict: macroState === "SOURCE_UNAVAILABLE" ? "NO_VOTE" : "NEUTRAL",
      claim_refs: macroState === "SOURCE_UNAVAILABLE" ? [] : [claimId],
      coverage_evidence_ids:
        macroEvidenceIds.length > 0 ? macroEvidenceIds : [evidenceId ?? `fake-${name}-snapshot`],
    },
    {
      criterion: "CATALYSTS",
      coverage_state: catalystState,
      comparison_status: catalystState === "SOURCE_UNAVAILABLE" ? "UNAVAILABLE" : "COMPARABLE",
      verdict: catalystState === "SOURCE_UNAVAILABLE" ? "NO_VOTE" : "NEUTRAL",
      claim_refs: catalystState === "SOURCE_UNAVAILABLE" ? [] : [claimId],
      coverage_evidence_ids:
        catalystEvidenceIds.length > 0
          ? catalystEvidenceIds
          : [evidenceId ?? `fake-${name}-snapshot`],
    },
...
function fakeSectorSubmission(name: StandardSectorAgentId, messages: unknown): unknown {
  const text = messageText(messages);
  const evidenceId = firstEvidenceId(text);
  const directive = jsonObjectAfterMarker(text, "Runtime final-selection directive:");
  const claimId = `fake-claim-${name}`;
  const directions = STANDARD_SECTOR_ROLE_CONTRACTS[name].directionIds;
  const preferredLocalId =
    typeof directive?.preferred_direction_id === "string"
      ? directive.preferred_direction_id
      : directions[0];
  const leastLocalId =
    typeof directive?.least_preferred_direction_id === "string"
      ? directive.least_preferred_direction_id
      : directions[directions.length - 1];
  const preferredSecurityIds = stringValues(directive?.allowed_preferred_security_ids);
  const leastSecurityIds = stringValues(directive?.allowed_least_preferred_security_ids);
  const requiredEvidenceIds = stringValues(directive?.required_final_evidence_ids);
  const claimEvidenceIds = [
    ...new Set([...(evidenceId ? [evidenceId] : []), ...requiredEvidenceIds]),
  ];
  const longPicks = preferredSecurityIds.slice(0, 1).map((tsCode, index) => ({
    pick_local_id: `fake-${name}-long-${index + 1}`,
    ts_code: tsCode,
    direction_local_id: preferredLocalId,
    position_action: "LONG",
    conviction: 0.5,
    thesis: "Frozen PIT score row supports the preferred security.",
    claim_refs: [claimId],
  }));
  const avoidPicks = leastSecurityIds.slice(0, 1).map((tsCode, index) => ({
    pick_local_id: `fake-${name}-avoid-${index + 1}`,
    ts_code: tsCode,
    direction_local_id: leastLocalId,
    position_action: "AVOID",
    conviction: 0.5,
    thesis: "Frozen PIT score row supports avoiding the least-preferred security.",
    claim_refs: [claimId],
  }));
  return {
    agent: name,
    ...
    claims: [
      {
        ...
        structured_conclusion: {
          ...
          target_local_ref: preferredLocalId,
          selection_status: "SELECTED",
          direction_id: preferredLocalId,
    ...
    claim_refs: [claimId],
    preferred_security_status: longPicks.length > 0 ? "PICKS_PRESENT" : "NO_QUALIFIED_SECURITY",
    preferred_security_abstention_confidence: longPicks.length > 0 ? null : 0.5,
    long_picks: longPicks,
    least_preferred_security_status:
      avoidPicks.length > 0 ? "PICKS_PRESENT" : "NO_QUALIFIED_SECURITY",
    least_preferred_security_abstention_confidence: avoidPicks.length > 0 ? null : 0.5,
    short_or_avoid_picks: avoidPicks,
    macro_input_attributions: MACRO_AGENT_IDS.map((macroAgentId) => ({
      agent_id: macroAgentId,
      target_type: "SUBMISSION_SUMMARY",
      target_local_ref: "$SUBMISSION",
...
function jsonObjectAfterMarker(text: string, marker: string): Record<string, unknown> | null {
...
function embeddedJsonObject(text: string, property: string): Record<string, unknown> | null {
  ...
  for (let index = start; index < text.length; index++) {
    ...
    if (character === '"') inString = true;
      ...
      if (depth === 0) {
        try {
          return objectRecord(JSON.parse(text.slice(start, index + 1)));
        } catch {
          return null;
        }
      }
