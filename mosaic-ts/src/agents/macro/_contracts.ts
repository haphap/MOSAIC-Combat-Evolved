...
> = {
  ...
  cohort_bull_2007: {
    zh: "检查流动性扩张是否伴随杠杆和尾部脆弱性。",
    en: "Test whether liquidity expansion also builds leverage and tail fragility.",
  },
  cohort_bull_2016: {
    zh: "区分可持续周期修复与短暂价格、库存或政策脉冲。",
    en: "Separate durable cyclical repair from temporary price, inventory, or policy impulses.",
  },
  cohort_crisis_2008: {
    zh: "检查融资、流动性、外需和资产负债表放大渠道。",
    en: "Check amplification through funding, liquidity, external demand, and balance sheets.",
  },
  cohort_crisis_covid: {
    zh: "区分暂时停摆、持续需求损伤、供应约束和政策托底。",
    en: "Separate temporary shutdowns, persistent demand damage, supply constraints, and policy support.",
  },
  cohort_euphoria_2021: {
    zh: "降低单一叙事确认的可信度，并识别反转触发器。",
    en: "Discount single-narrative confirmation and identify reversal triggers.",
  },
  cohort_rate_tightening: {
    zh: "检查收紧冲击的持续性、时滞和非对称传导。",
    en: "Test the persistence, lag, and asymmetry of tightening shocks.",
  },
  cohort_recovery_2020: {
    zh: "区分低基数反弹、连续修复和可持续加速。",
    en: "Separate low-base rebounds, sequential repair, and durable acceleration.",
  },
};
...
export const MACRO_ROLE_CONTRACTS: Readonly<Record<MacroAgentId, MacroRoleContract>> = {
  ...
  us_financial_conditions: {
    ...
    prohibited: {
      zh: ["不得再投一张美国经济周期票", "不得把 Fed、美元、曲线拆成多票"],
      en: [
  ...
  euro_area_financial_conditions: {
    ...
    prohibited: {
      zh: ["不得重复欧盟实体周期", "不得纳入非欧元区央行或市场"],
      en: [
        "Do not repeat the EU real-economy cycle",
...
const signalTailFields = {
  ...
  channels: z.array(z.string().trim().min(1)).min(1).max(8),
...
const MacroClaimSchema = ClaimSchemaV2.safeExtend({
  structured_conclusion: z
    .object({
      ...
      a_share_transmission: z.string().trim().min(1).max(320),
    })
    .strict(),
});

export const MACRO_SUBMISSION_FIELD_NAMES = [
  "mode",
  "claims",
  "key_drivers",
  "signal",
  ...
  const exactComponents =
    expectedComponents.length > 0
      ? (z.tuple(
        ...
        ) as unknown as z.ZodType<z.infer<typeof MacroComponentSignalSchema>[]>)
      : z.tuple([]);
  const common = {
    claims: z.array(MacroClaimSchema).min(1).max(8),
    key_drivers: z.array(z.string().trim().min(1)).min(1).max(8),
  };
  const schema =
...
export function renderMacroPromptBody(
  ...
  if (language === "zh") {
    return [
      ...
      `提交 mode=${role.mode}；direction、strength、persistence_horizon、evaluation_horizon_trading_days、confidence、channels、claims、claim_refs 和 key_drivers 必须符合运行时 schema。`,
  ...
  return [
    ...
    `Submit mode=${role.mode}; direction, strength, persistence_horizon, evaluation_horizon_trading_days, confidence, channels, claims, claim_refs, and key_drivers must follow the runtime schema.`,
    ...(components.length > 0 ? [`components must be exactly: ${components.join(", ")}.`] : []),
