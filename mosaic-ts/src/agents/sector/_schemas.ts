...
const SectorFinalClaimSchema = ClaimSchemaV2.safeExtend({
  structured_conclusion: z
    .object({
      ...
      selection_status: z.enum(["SELECTED", "NO_QUALIFIED_DIRECTION"]).nullable(),
...
export function buildStandardSectorSchema<TAgent extends StandardSectorAgentId>(
  agent: TAgent,
  requiredSelectionStatus?: "SELECTED" | "NO_QUALIFIED_DIRECTION",
  directive?: {
  ...
  const singleRegisteredDirection = directionIds.length === 1;
  const preferredDirection =
    directive?.selection_status === "SELECTED" && directive.preferred_direction_id
      ? z.literal(directive.preferred_direction_id)
      : direction;
  const preferredSecurityUnavailable =
    directive?.selection_status === "SELECTED" &&
    directive.allowed_preferred_security_ids.length === 0;
  const leastSecurityUnavailable =
    directive?.selection_status === "SELECTED" &&
    directive.least_preferred_status === "REQUIRED" &&
    directive.allowed_least_preferred_security_ids.length === 0;
  const noQualifiedLeastReason =
    directive && directive.least_preferred_status !== "REQUIRED"
      ? (directive.least_preferred_reason as Exclude<
          typeof directive.least_preferred_reason,
          "UNIQUE_CONDORCET_LOSER"
        >)
      : undefined;
  const leastPreferredDirection =
    directive?.selection_status === "SELECTED" &&
    directive.least_preferred_status === "REQUIRED" &&
    directive.least_preferred_direction_id
      ? z
          .object({
            ...
            direction_id: z.literal(directive.least_preferred_direction_id),
          ...
          .strict()
      : noQualifiedAvoidSchema(noQualifiedLeastReason);
  const selected = z
    .object({
      ...
      preferred_direction: z
        ...
        .strict(),
      least_preferred_direction: directive
        ? leastPreferredDirection
        : singleRegisteredDirection
          ? noQualifiedAvoidSchema()
          : z.union([
              z
                .object({
                  selection_role: z.literal("LEAST_PREFERRED"),
      ...
      long_picks: preferredSecurityUnavailable ? z.tuple([]) : z.array(SecurityPick).max(5),
      least_preferred_security_status:
        directive?.selection_status === "SELECTED" &&
        directive.least_preferred_status !== "REQUIRED"
          ? z.literal("NOT_APPLICABLE")
          : leastSecurityUnavailable
            ? z.literal("NO_QUALIFIED_SECURITY")
            : singleRegisteredDirection
              ? z.literal("NOT_APPLICABLE")
              : z.enum(["PICKS_PRESENT", "NO_QUALIFIED_SECURITY", "NOT_APPLICABLE"]),
      ...
      short_or_avoid_picks:
        directive?.selection_status === "SELECTED" &&
        (directive.least_preferred_status !== "REQUIRED" || leastSecurityUnavailable)
          ? z.tuple([])
          : singleRegisteredDirection
            ? z.tuple([])
  ...
  const noQualified = z
    .object({
      agent: z.literal(agent),
      selection_status: z.literal("NO_QUALIFIED_DIRECTION"),
      preferred_direction: z.object({ status: z.literal("NO_QUALIFIED_DIRECTION") }).strict(),
      least_preferred_direction: noQualifiedAvoidSchema(noQualifiedLeastReason),
      ...common,
      preferred_security_status: z.literal("NO_QUALIFIED_SECURITY"),
      preferred_security_abstention_confidence: z.null(),
      long_picks: z.tuple([]),
      least_preferred_security_status: z.literal("NOT_APPLICABLE"),
      least_preferred_security_abstention_confidence: z.null(),
      short_or_avoid_picks: z.tuple([]),
    })
    .strict();
  const runtimeSchema =
    requiredSelectionStatus === "SELECTED"
      ? selected
      : requiredSelectionStatus === "NO_QUALIFIED_DIRECTION"
        ? noQualified
        : z.union([selected, noQualified]);
  return runtimeSchema.superRefine((output, ctx) => {
    const claimIds = new Set(output.claims.map((claim) => claim.claim_id));
    for (const ref of output.claim_refs) {
      if (!claimIds.has(ref))
        ctx.addIssue({ code: "custom", path: ["claim_refs"], message: `unknown claim_ref ${ref}` });
    }
    const validAttributionTargets = new Set<string>();
    if (output.selection_status === "SELECTED") {
      validAttributionTargets.add(
    ...
    if (output.selection_status === "SELECTED") {
      ...
      if (output.short_or_avoid_picks.reduce((sum, pick) => sum + pick.conviction, 0) > 1 + 1e-9) {
        ctx.addIssue({
          code: "custom",
      ...
      if ("direction_local_id" in output.least_preferred_direction) {
        ...
        output.least_preferred_security_status !== "NOT_APPLICABLE" ||
        output.least_preferred_security_abstention_confidence !== null ||
        output.short_or_avoid_picks.length !== 0
      ) {
        ctx.addIssue({
          code: "custom",
          path: ["least_preferred_security_status"],
          message: "unqualified least-preferred direction requires a non-applicable security leg",
        });
      }
    }
    const entryRefs = [
      ...
      ...(output.selection_status === "SELECTED"
        ? [
            ...
            ...("claim_refs" in output.least_preferred_direction
...
function validateSecurityLeg(
  ...
  } else if (!input.notApplicableAllowed) {
    ctx.addIssue({
      code: "custom",
      path: [path],
      message: "NOT_APPLICABLE is not allowed for this qualified direction",
    });
  }
...
function noQualifiedAvoidSchema(
  requiredReason?:
    | "SINGLE_ELIGIBLE_DIRECTION"
    | "PREFERRED_NOT_QUALIFIED"
    | "NO_UNIQUE_CONDORCET_LOSER"
  ...
  return z
    .object({
      status: z.literal("NO_QUALIFIED_AVOID_DIRECTION"),
      reason: requiredReason
        ? z.literal(requiredReason)
        : z.enum([
            "SINGLE_ELIGIBLE_DIRECTION",
            "PREFERRED_NOT_QUALIFIED",
            "NO_UNIQUE_CONDORCET_LOSER",
