...
function buildRepairMessages<T>(input: {
  ...
  if (input.attempt === 1) {
    return [
      new SystemMessage(
        "Structured repair 1/3. Correct the complete prior object directly. Return a complete object; do not omit disposition fields or add prose. Copy exact evidence_id and allowed_research_rule_ids from the immutable catalog: every claim needs evidence_ids, and every INTERPRETATION also needs research_rule_refs.",
      ),
      new HumanMessage(
        JSON.stringify({
...
function extractRepairEvidenceCatalog(snapshot: unknown): RepairEvidenceCatalog {
  if (snapshot === null || typeof snapshot !== "object" || Array.isArray(snapshot)) {
    return { allowed_evidence_ids: [], allowed_research_rule_ids: [] };
  }
  ...
  return {
    ...
    allowed_research_rule_ids: [...new Set(allowedResearchRuleIds)],
  };
}
