...
const StandardSectorSecurityLegOutcomeMetricSchema = z
  .object({
    side: z.enum(["PREFERRED", "LEAST_PREFERRED"]),
    direction_id: NonEmptyId,
    security_status: z.enum(["PICKS_PRESENT", "NO_QUALIFIED_SECURITY_EMPTY_SHORTLIST"]),
...
export const StandardSectorOutcomeRawMetricsSchema = z
  .object({
    ...
    confidence_semantics: z.literal("DIRECTIONAL_UTILITY"),
    ...
    security_leg_metrics: z.array(StandardSectorSecurityLegOutcomeMetricSchema).length(2),
    ...
    unit_confidence_utility_delta: FiniteNumber,
    confidence_calibration_target: z.union([z.literal(0), z.literal(1)]),
  })
  .strict()
  .superRefine((value, ctx) => {
    const legs = new Map(value.security_leg_metrics.map((row) => [row.side, row]));
    if (legs.size !== 2) {
      ctx.addIssue({
        code: "custom",
        path: ["security_leg_metrics"],
        message: "legs must be unique",
      });
      return;
    }
    for (const [side, leg] of legs) {
      const securityMetrics = value.security_metrics.filter((row) => row.side === side);
      const empty = leg.security_status === "NO_QUALIFIED_SECURITY_EMPTY_SHORTLIST";
      if ((empty && securityMetrics.length !== 0) || (!empty && securityMetrics.length === 0)) {
        ctx.addIssue({
          code: "custom",
          path: ["security_leg_metrics"],
          message: "security metrics must match each frozen shortlist disposition",
        });
      }
      const selectedRole = side === "PREFERRED" ? "PREFERRED" : "LEAST_PREFERRED";
      const selectedDirections = value.direction_metrics.filter(
        (row) => row.selected_role === selectedRole,
      );
      if (
        selectedDirections.length !== 1 ||
        selectedDirections[0]?.direction_id !== leg.direction_id ||
        securityMetrics.some((row) => row.direction_id !== leg.direction_id)
      ) {
        ctx.addIssue({
          code: "custom",
          path: ["security_leg_metrics"],
          message: "security legs must bind the exact selected directions",
        });
      }
    }
  });
...
const sector = (agent_id: string): OutcomeContract => ({
  ...
  outcome_contract_version: "sector_direction_pick_outcome_v3",
  scoring_contract_version: `score_${agent_id}_direction_pick_v3`,
  ...
  opportunity_set_contract_version: `${agent_id}_sector_opportunity_set_v3`,
  normalization_contract_version: "sector_opportunity_adjusted_utility_normalization_v3",
  required_source_ids: [
    "sector_universe_manifest_v1",
    "sector_research_snapshot_v4",
