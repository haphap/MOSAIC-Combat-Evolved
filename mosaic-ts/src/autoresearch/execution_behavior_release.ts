...
import { upsertRuntimeEvidenceContract } from "../agents/prompts/research_knobs_projection.js";
...
import {
  ...
  SingleDirectionQualificationSubmissionSchema,
...
import { KNOT_RUNTIME_CONTRACT_MANIFEST } from "./knot_contract.js";
...
export function buildExecutionBehaviorReleaseManifest(
  ...
  const privatePromptCommit = requiredCommit(input.privatePromptCommit);
...
function buildVariant(
  ...
  const expected = expectedPrompt(input.agent, input.cohort, input.language);
  ...
  const canonicalDefault = expectedPrompt(input.agent, "cohort_default", input.language);
...
function expectedPrompt(agent: string, cohort: string, language: Language): string {
    ...
    return [
      schemaBinding(
        ...
        `${agent}.direction_research.v2`,
        {
          ...
          single_direction_schema: toJsonSchema(SingleDirectionQualificationSubmissionSchema),
        },
        language,
      ),
      schemaBinding(
...
function computeStructuredProviderContractHash(agent: string): string {
  const runtimeDomainContract = STANDARD_SECTOR_IDS.includes(agent)
    ? {
        ...
        direction_research: "EXACT_PAIR_OR_SINGLE_DIRECTION_DOMAIN_V1",
